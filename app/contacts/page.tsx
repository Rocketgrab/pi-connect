  ...
  return (
    <div>
      ...
      <p className="mt-3 max-w-3xl text-[1.1rem] leading-relaxed text-ink">
        Direct emails are included only where they appear on a public claims
        sheet, team page or association site. Where a personal email is not
        published, use the switchboard and ask for the claims or PI lead by
        title.
      </p>
            ...
            <li key={event.id} className="rounded-xl border border-line bg-white p-5">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-[1.18rem] font-medium">{event.name}</p>
      ...
      <section className="mt-12 space-y-5">
        ...
        {organisations.map((org) => (
          <article key={org.id} className="rounded-xl border border-line bg-white p-5 sm:p-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <p className="text-[0.85rem] uppercase tracking-[0.1em] text-ink-soft">
                  {org.type}
                </p>
                <h3 className="mt-1 text-[1.28rem] font-medium">{org.name}</h3>
              </div>
              <div className="shrink-0 rounded-lg border border-line bg-paper px-3 py-2">
            ...
            <ul className="mt-4 divide-y divide-line border-t border-line">
              {org.people.map((person) => (
                <li key={`${org.id}-${person.name}`} className="py-3">
                  ...
                  <p className="text-[1.02rem] text-ink-soft">{person.role}</p>
            ...
            {org.notes ? (
              <p className="mt-3 text-[1.02rem] text-ink-soft">{org.notes}</p>
            ) : null}
          </article>
        ))}
      </section>
